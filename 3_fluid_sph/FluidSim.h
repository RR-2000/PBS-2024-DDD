...
#pragma region FluidSteps
	...
	void initializeParticles(int numParticles, double domainSize) {
		m_particles.clear();
		for (int i = 0; i < numParticles; ++i) {
			Eigen::Vector2d pos = Eigen::Vector2d::Random() * domainSize;
			Eigen::Vector2d vel = Eigen::Vector2d::Zero();
			m_particles.emplace_back(pos, vel, 1.0); // mass = 1.0
		}
	}


	double poly6Kernel(const Eigen::Vector2d& r, double h) const {
		double r_len = r.norm();
		if (r_len >= 0 && r_len <= h) {
	...
	Eigen::Vector2d spikyGradient(const Eigen::Vector2d& r, double h) const {
		double r_len = r.norm();
		if (r_len > 0 && r_len <= h) {
			double factor = -45.0 / (M_PI * pow(h, 6));
			return factor * pow(h - r_len, 2) * (r / r_len);
		}
	...
	void computeDensityPressure() {
		for (auto& pi : m_particles) {
			...
			for (const auto& pj : m_particles) {
				Eigen::Vector2d r = pi.position - pj.position;
				pi.density += pj.mass * poly6Kernel(r, m_sphKernelRadius);
			}
			pi.pressure = m_stiffness * (pow(pi.density / m_restDensity, m_gamma) - 1.0);
		}
	}

	void computeForces() {
		for (auto& pi : m_particles) {
			Eigen::Vector2d pressureForce = Eigen::Vector2d::Zero();
			Eigen::Vector2d viscosityForce = Eigen::Vector2d::Zero();
			...
			for (const auto& pj : m_particles) {
				...
				// Pressure force
				pressureForce += pj.mass * (pi.pressure / (pi.density * pi.density) +
											pj.pressure / (pj.density * pj.density)) *
								spikyGradient(r, m_sphKernelRadius);

				// Viscosity force
				viscosityForce += m_viscosity * pj.mass * (pj.velocity - pi.velocity) / pj.density;
			}

			// Apply forces
			pi.acceleration = -pressureForce + viscosityForce + m_gravity;
		}
	}
	...
	void integrateParticles() {
		for (auto& p : m_particles) {
			p.velocity += m_dt * p.acceleration;
			p.position += m_dt * p.velocity;
		}
	}
...
private:
	...
	struct Particle {
		...
		Particle(Eigen::Vector2d pos, Eigen::Vector2d vel, double m)
			: position(pos), velocity(vel), mass(m) {}
	};


	std::vector<Particle> m_particles;  // SPH particles
    double m_sphKernelRadius = 1.0;     // Kernel radius (h)
    double m_restDensity = 1000.0;      // Rest density (ρ₀)
    double m_viscosity = 0.1;           // Viscosity coefficient
    double m_stiffness = 3.0;           // Stiffness for pressure (k)
    double m_gamma = 7.0;               // Exponent for EOS
    Eigen::Vector2d m_gravity = {0.0, -9.81};  // Gravity vector
    bool m_useSPH = false;              // Toggle between grid and SPH
