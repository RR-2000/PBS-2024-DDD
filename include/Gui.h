...
protected:
	...
	void addParticlesToViewer();

	void setSimSpeed(int speed) { m_simSpeed = speed; }
	int getSimSpeed() const { return m_simSpeed; }
	void setFastForward(bool fast) { 
		m_fastForward = fast;
		assert(p_simulator != nullptr);
		if (m_fastForward) 
			p_simulator->setSimulationSpeed(m_maxSimSpeed);
		else
			p_simulator->setSimulationSpeed(m_simSpeed);
	}
	int getFastForward() const { return m_fastForward; }
